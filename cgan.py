...
def get_batch_train(numExamples, batchSize, X_train, Y_train, X_noise, Y_noise, generator):

    noiseList = np.random.randint(numExamples, size = batchSize)
    X_noise_sampled, Y_noise_sampled = X_noise[noiseList], Y_noise[noiseList]

    noiseData_X, noiseData_Y = generator.predict([X_noise_sampled, Y_noise_sampled]), Y_noise[noiseList] #For discriminator

    ImageList = np.random.randint(numExamples, size = batchSize)
    X_train_sampled, Y_train_sampled = X_train[ImageList], Y_train[ImageList]

    noiseLabelsBatch, origLabelsBatch = np.zeros(batchSize).astype(int), np.ones(batchSize).astype(int)

    trainBatch_X = np.concatenate((noiseData_X, X_train_sampled), axis = 0)
    trainBatch_Y = np.concatenate((noiseData_Y, Y_train_sampled), axis = 0)
    trainLabel = np.concatenate((noiseLabelsBatch, origLabelsBatch), axis = 0)
    return trainBatch_X, trainBatch_Y, trainLabel

def get_batch_train_noise(numExamples, batchSize, X_noise, Y_noise):

    noiseList = np.random.randint(numExamples, size = batchSize)
    X_noise_sampled, Y_noise_sampled = X_noise[noiseList], Y_noise[noiseList]
    return X_noise_sampled, Y_noise_sampled
...
if __name__ == '__main__':
    ...
    for epoch in range(1, nbEpoch + 1):
        ...
        for i in range(numBatches):
            ...
            trainBatch_X, trainBatch_Y, trainLabel = get_batch_train(numExamples, batchSize, X_train, Y_train, X_noise, Y_noise, generator)
            
            trainBatch_X, trainBatch_Y, trainLabel = shuffle(trainBatch_X, trainBatch_Y, trainLabel)

            discriminatorLoss = discriminator.train_on_batch([trainBatch_X, trainBatch_Y], trainLabel)

            cganLabels = np.ones(batchSize).astype(int)

            discriminator.trainable = False

            X_noise_sampled, Y_noise_sampled = get_batch_train_noise(numExamples, batchSize, X_noise, Y_noise)

            dcganLoss = cgan.train_on_batch([X_noise_sampled, Y_noise_sampled], cganLabels)
            discriminator.trainable = True
